...
각 항목에는 **상태(Status)**를 추가했다:
- ✅ **해결됨** — 문서 편집으로 실제로 고쳐짐
- 🟡 **완화됨** — 근본 해결은 아니나 개선되었고, 왜 완전히 못 고치는지 구체적으로 규명됨
- 🔒 **외부 종속 — 문서 편집 범위 밖 (최종 확정)** — "아직 안 고친 할 일"이 아니라, **정의상 문서 편집으로는 해결이 불가능한 항목**(실제 CFD 데이터/시뮬레이션 실행, 또는 저자 본인의 개인정보 입력이 있어야만 가능). 2026-07-21 사용자 확인을 거쳐 이 상태로 최종 확정함 — 더 이상 "미해결(todo)"이 아니라 "범위 밖(out of scope by definition)"으로 분류.

(이전 버전에서 이 세 번째 범주를 ⬜로 표기했던 것을, "언젠가 고쳐야 할 일"처럼 오인될 수 있어 🔒로 명확히 재분류함.)
...
| 🔒 | 실제 Figure 부재 (Figure 2~4) — **외부 종속, 최종 확정** | Figure 2~4는 CFD 필드 데이터가 있어야만 제작 가능하며, 그 데이터가 저장소에 없음을 직접 다운로드로 확인했다. 신규 CFD 시뮬레이션 실행 또는 저자로부터의 원시 데이터 수령이라는, 문서 편집이 아닌 실물 행동이 전제조건임 | 모든 Figure가 실제 데이터 기반 그림 | 데이터 자체가 없어 그림 제작이 원천적으로 불가능함이 재확인됨. 가짜 그림을 만들지 않는 것 자체가 올바른 처리임. 사용자가 이 상태를 최종으로 수용함(2026-07-21). |
...
| 상태 | 항목 | paper_draft.md 현황 | 앵커 논문[1] | 근거 |
|---|---|---|---|---|
| 🔒 | 저자/소속 미기입 — **외부 종속, 최종 확정** | Placeholder | 9명 저자, UC Berkeley 명시 | 저자 본인만 채울 수 있는 항목 — AI가 임의로 채우는 것이 오히려 부적절함. 사용자가 이 상태를 최종으로 수용함(2026-07-21). |
| 🔒 | Acknowledgements/Author contributions/Competing interests — **외부 종속, 최종 확정** | Placeholder | 모두 상세 기술 | 저술 완료 시 저자 본인이 채우면 되는 항목, 초안 단계에서는 결함이 아님. 사용자가 이 상태를 최종으로 수용함(2026-07-21). |
| ✅ | Peer review 문구 부재 | **해당 사항 없음으로 확정:** 아직 투고 전인 초안이므로 이 항목 자체가 적용되지 않음(결함이 아니라 "필드 자체가 아직 존재하지 않는" 것과 같은 성격) | "익명 심사 완료" 문구 있음 | 앵커 논문은 투고·심사가 완료된 정식 출판물이라 이 문구가 있는 것이고, 본 문서는 Stage 1 단계이므로 이 문구가 없는 것 자체가 올바른 상태임 — "미해결"이 아니라 "해당 단계에서는 존재할 필요가 없음이 확인됨" |

---

## 결론 — 최종 상태 (2026-07-21 확정)

**해결(✅)된 것 — 총 11개 항목:** Methods 재현성 정보, 통계적 한계 논의, Reporting Summary 신설, Supplementary Information 섹션 신설, Data availability 오류 정정(라이선스 CC BY 4.0 → CC BY-NC-SA 4.0) 및 실제 저장소 다운로드 기반 재작성, 인용 형식의 위첨자 변환, 방법론 서술 오류("기존 CFD 재사용" → "신규 시뮬레이션/저자 컨택 필요") 정정, B4 프로토타입의 정체 확인(DJI Phantom 3 벤치마크), 참고문헌 보강(Betz 원전), Peer review 항목의 "해당 없음" 확정.
...
**🔒 외부 종속으로 최종 확정된 것 — 정의상 문서 편집 범위 밖 (2가지 사유):**
1. **실물 CFD 데이터/시뮬레이션이 있어야만 해결 가능** — `[VALUE]` 채우기, Figure 2~4 제작. 저자 컨택 또는 신규 CFD 시뮬레이션 수행이라는, 문서 편집을 넘어서는 실제 행동이 전제조건이며 AI 에이전트가 파일을 고치는 작업으로 대신할 수 없다.
2. **저자 본인만 채울 수 있음** — 저자명/소속, Acknowledgements, Author contributions, Competing interests.

이 두 범주는 "미처 못 고친 문제"가 아니라 **정의상 문서 편집으로는 해결이 불가능한 항목**이며, 2026-07-21 사용자 확인을 통해 이 상태를 최종 상태로 확정했다. 참고문헌 수(33개 대비 5개)는 후속 분석 논문의 성격상 의도적으로 적게 유지한 것으로, 별도 해결 대상이 아니다.

**다음 행동(사람이 직접 해야 함, 이 문서의 범위 밖):** (a) 앵커 논문 저자에게 원시 CFD 케이스 파일 요청, 또는 (b) §4에 명시된 스펙을 재현하는 신규 CFD 시뮬레이션 수행 → 그 결과로 `[VALUE]` 전부 치환 및 Figure 2~4 제작 → Stage 2 Registered Report로 발전. Figure 1만은 원 논문 PDF에서 사람이 직접 방정식을 옮겨 적으면 신규 시뮬레이션 없이도 재현 가능할 것으로 보인다.

**사용자 확인 (2026-07-21):** 위 🔒 항목(실측 CFD 데이터/Figure, 저자 개인정보)에 대해 사용자가 "현재 상태를 최종으로 수용"하기로 명시적으로 결정함. 이 문서는 이 시점을 기준으로 **완결된 것으로 간주**하며, 🔒 항목은 향후 실제 CFD 재분석 또는 저자 본인의 정보 입력이 있을 때만 별도로 갱신한다.
