...
| ⬜ | 실제 Figure 부재 | Figure 1~4가 텍스트 placeholder만 존재, 실제 이미지 없음 (상세 명세는 아래 "Figure 상세 설명" 섹션) | 모든 Figure가 실제 데이터 기반 그림 | 위와 동일한 이유로 데이터 자체가 없어 그림 제작이 원천적으로 불가능함이 이번에 재확인됨. 가짜 그림을 만들지 않는 것 자체가 올바른 처리임. |
...
| 🆕 ⬜ | (신규 발견) 프로토타입 개수 불일치 (4개 vs 실제 5개, B4 존재) | §2 본문 범위는 여전히 4개(3D-SC, B1, B2, B3)로 유지하되, B4의 존재와 제외 사유가 불명확함을 §4·Abstract·Intro·SI 계획에 명시 | 원 논문/저장소에 B4가 실제로 존재 | B4가 무엇인지(추가 baseline인지, 실패한 variant인지 등)는 저장소 파일만으로는 알 수 없어 저자 확인이 필요 — AI가 임의로 추정해 채우면 오히려 오류 |
...
**이번 라운드(2차)에서 해결(✅)한 것:** Supplementary Information 섹션 신설, Data availability 오류 정정(라이선스 CC BY 4.0 → CC BY-NC-SA 4.0) 및 실제 저장소 다운로드 기반 재작성, 인용 형식의 위첨자 변환 — 3개 항목. (1차에서 해결한 Methods 재현성/통계적 한계/Reporting Summary/참고문헌 보강까지 합쳐 총 8개 항목 해결.)

**이번 라운드의 가장 중요한 성과:** 실측 데이터 부재(Critical)의 원인을 막연히 "재분석 필요"라고 남겨두는 대신, **앵커 논문의 실제 데이터 저장소를 직접 다운로드하여 검사**함으로써 (1) 원시 CFD 필드 데이터가 애초에 공개되어 있지 않다는 것, (2) 프로토타입이 4개가 아니라 5개(B4 존재)라는 것, (3) 라이선스가 CC BY-NC-SA 4.0이라는 것을 **검증된 사실**로 확정했다. 이에 따라 §4 Methods, Abstract, Introduction의 "기존 CFD 해를 재사용하면 된다"는 서술 전체를 "저자 컨택 또는 신규 CFD 시뮬레이션이 필요하다"로 정정했다 — 이는 지어낸 완화가 아니라 실제 조사에 기반한 방법론 수정이다.

**그럼에도 구조적으로 완전히 해결할 수 없는 것 (2가지 이유):**
1. **실물 CFD 데이터/시뮬레이션이 있어야만 해결 가능** — `[VALUE]` 채우기, 실제 Figure 제작, Supplementary Information의 실제 내용. 이는 저자 컨택 또는 신규 CFD 시뮬레이션 수행이라는, 문서 편집을 넘어서는 실제 행동이 필요하며 AI 에이전트가 파일을 고치는 작업만으로는 대신할 수 없다.
...
**다음 행동(사람이 직접 해야 함):** (a) 앵커 논문 저자에게 원시 CFD 케이스 파일 요청, 또는 (b) §4에 명시된 스펙을 재현하는 신규 CFD 시뮬레이션 수행 → 그 결과로 `[VALUE]` 전부 치환 및 Figure 1~4 제작 → Stage 2 Registered Report로 발전.
